...
class Autopilot {
	...
	toggle() {
		...
		if (!vehicle) {
			return mp.game.ui.notifications.show('error', 'Сядьте у ТЗ');
		}
	...
	private start(vehicle: VehicleMp) {
		if (!waypoint.isActive()) {
			return mp.game.ui.notifications.show('error', 'Потрiбно вказати точку на картi');
		}

		if (!this.isAvailableVehicle(vehicle)) {
			return mp.game.ui.notifications.show(
				'error',
				'Дане ТЗ не має пiдтримки автопiлота.'
			);
		}
		...
		mp.game.ui.notifications.show('info', 'Автопiлот активовано');
	}
	...
	private stop(vehicle: VehicleMp) {
		...
		mp.game.ui.notifications.show('info', 'Автопiлот вимкнено');
	}
